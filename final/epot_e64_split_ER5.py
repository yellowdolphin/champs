...
learning_rate = 2e-4   # after conv to ~ 0.0025 (round 2), lower from 2e-4 to 0.8e-4
epochs = 10
chk_period = 10
val_period = 5
batch_size = 32
...
reg_dim = 256
T = 4
datadir = '../data/champs-complete-graph/'
...
print("Labels J-coupling train:", len(y_train[0]))
print("Labels E_pot train:", len(y_train[1]))
print("Labels J-coupling dev:", len(y_dev[0]))
print("Labels E_pot dev:", len(y_dev[1]))
train_sequence = MultiSequence(input_data['inputs_train'], y_train, batch_size)
valid_sequence = MultiSequence(input_data['inputs_valid'], y_dev, batch_size)
...
# 1D inputs -> scalars
snode_graph_indices = Squeeze(name='squeeze_node_graph_indices')(node_graph_indices)
...
# Readout for total energy.
X = Dense(state_dim//2, activation='softplus')(atom_state)
X = Dense(1)(X)
e_pot = ReduceAtomToMol(reducer='sum', name='E_pot')([X, snode_graph_indices])
...
concat = Concatenate(name='ER5_concat')([edge_features, edge_indices, bond_state])
...
get_bond_state = Lambda(lambda x: x[:, n_edge_features+1:n_edge_features+1+state_dim])
...
output_1JHC = ER5_without_atoms(edge_features_1JHC, bond_state_1JHC, "1JHC", reg_dim)
output_1JHN = ER5_without_atoms(edge_features_1JHN, bond_state_1JHN, "1JHN", reg_dim)
output_otherJ = ER5_without_atoms(edge_features_otherJ, bond_state_otherJ, "otherJ", reg_dim)

# Skip Readout for X when edge_features_X is empty (J-class X does not occur in batch).
# This does not work: both edge_features and output_X are evaluated, the latter crashes on empty batch input.
output_1JHC = Lambda(skip_X, name='skip_1JHC')([edge_features_1JHC, output_1JHC])
output_1JHN = Lambda(skip_X, name='skip_1JHN')([edge_features_1JHN, output_1JHN])
output_otherJ = Lambda(skip_X, name='skip_otherJ')([edge_features_otherJ, output_otherJ])

# Simulate output for the irrelevant bonds without labels and for empty slots.
...
output = Lambda(idx_join, name='sER5_ouput')([output_1JHC, output_1JHN, output_otherJ, output_rest])
...
# If a list of losses is specified, one for each output, their sum will be minimized (joint training).
model.compile(optimizer=Adam(lr=learning_rate), loss=[masked_log_mae_1d, energy_mse], metrics=[masked_mae_1d])
